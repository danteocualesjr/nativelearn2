...
final class CompanionManager: ObservableObject {
    ...
    /// User preference for whether the Sparkle cursor should be shown.
    ...
    func start() {
        ...
        print("🔑 Sparkle start — accessibility: \(hasAccessibilityPermission), screen: \(hasScreenRecordingPermission), mic: \(hasMicrophonePermission), screenContent: \(hasScreenContentPermission), onboarded: \(hasCompletedOnboarding)")
    ...
    private func startOnboardingMusic() {
        ...
        guard let musicURL = Bundle.main.url(forResource: "ff", withExtension: "mp3") else {
            print("⚠️ Sparkle: ff.mp3 not found in bundle")
            return
        }
        ...
        do {
            ...
            print("⚠️ Sparkle: Failed to play onboarding music: \(error)")
        }
    }
    ...
    private func sendTranscriptToClaudeWithScreenshot(transcript: String) {
        ...
        currentResponseTask = Task {
            ...
            do {
                ...
                // Kick off (don't await) a background pass that asks Haiku to
                // generate a polished title and one-sentence summary for the
                // conversation. The voice response has already finished —
                // metadata generation runs out-of-band so it never blocks
                // playback. The fallback heuristic title set inside
                // `appendExchange` stays visible until this completes.
                if let activeId = conversationStore?.activeConversationId {
                    regenerateConversationMetadataIfNeeded(
                        conversationId: activeId,
                        exchangeCountAfterAppend: totalExchangesAfterAppend
                    )
                }
    ...
    // MARK: - Conversation Metadata (Auto Title & Summary)

    /// System prompt used for the lightweight Haiku call that generates a
    /// conversation title and one-sentence summary. The output is parsed
    /// strictly as JSON, so the prompt heavily constrains the response shape.
    private static let conversationMetadataSystemPrompt = """
    you generate a concise title and one-sentence summary for a voice conversation between a user and sparkle, an AI tutor that teaches AI tools.

    rules:
    - title: 3 to 6 words, lowercase, no trailing punctuation, no quotes. it should describe the topic, not be a question. examples: "cursor compose mode basics", "replit deployment walkthrough", "claude code project setup".
    - summary: one short sentence (max 14 words) describing what the conversation is about. lowercase. no quotes. example: "user is learning how to use cursor's compose mode for refactoring."
    - if the conversation is too short or unclear, do your best with what's there. do not refuse.

    respond with ONLY a JSON object on a single line, exactly this shape:
    {"title":"...","summary":"..."}

    no other text, no code fences, no commentary.
    """

    /// Set of exchange counts at which we (re)generate metadata. The first
    /// run gives the conversation a real title fast; the later runs refine
    /// title and summary as the topic firms up. Keeping the set small bounds
    /// the cost so a long session doesn't keep paying for renames.
    private static let exchangeCountsForMetadataRefresh: Set<Int> = [1, 3, 6, 12]

    /// Cancels any prior metadata regeneration so two background calls never
    /// race on the same conversation. The latest exchange always wins.
    private var pendingMetadataRegenerationTask: Task<Void, Never>?

    /// Fires a background Claude call (Haiku — cheap and fast) to derive a
    /// nicer title and summary from the latest exchanges. The method is
    /// fire-and-forget: it never throws, never blocks the voice pipeline, and
    /// silently no-ops on errors. Results are written through
    /// `applyAutoGeneratedMetadata`, which respects the user's manual rename
    /// flag so AI-generated text never clobbers a name the user chose.
    private func regenerateConversationMetadataIfNeeded(
        conversationId: UUID,
        exchangeCountAfterAppend: Int
    ) {
        guard Self.exchangeCountsForMetadataRefresh.contains(exchangeCountAfterAppend) else { return }

        pendingMetadataRegenerationTask?.cancel()
        pendingMetadataRegenerationTask = Task { [weak self] in
            await self?.runConversationMetadataRegeneration(conversationId: conversationId)
        }
    }

    private func runConversationMetadataRegeneration(conversationId: UUID) async {
        guard let conversation = conversationStore?.conversations.first(where: { $0.id == conversationId })
        else { return }
        if conversation.titleWasManuallyEdited { return }
        if conversation.exchanges.isEmpty { return }

        // Build a compact context window: always include the very first
        // exchange (so the title stays anchored to original intent) plus the
        // last 8 exchanges (so the summary tracks what's currently happening).
        let firstExchange = conversation.exchanges.first
        let recentExchanges = Array(conversation.exchanges.suffix(8))
        var contextExchanges: [ConversationExchange] = []
        if let first = firstExchange,
           !recentExchanges.contains(where: { $0.id == first.id }) {
            contextExchanges.append(first)
        }
        contextExchanges.append(contentsOf: recentExchanges)

        let userPromptText = Self.buildMetadataUserPrompt(from: contextExchanges)

        do {
            let rawResponseText = try await claudeAPI.generateText(
                systemPrompt: Self.conversationMetadataSystemPrompt,
            ...
            guard let parsedMetadata = Self.parseMetadataJSON(from: rawResponseText) else {
                print("⚠️ Conversation metadata: could not parse response — \(rawResponseText.prefix(160))")
                return
            }

            conversationStore?.applyAutoGeneratedMetadata(
                conversationId: conversationId,
                title: parsedMetadata.title,
                summary: parsedMetadata.summary
            )
            print("✨ Auto-titled conversation: \"\(parsedMetadata.title)\" — \"\(parsedMetadata.summary)\"")
        } catch is CancellationError {
            return
        } catch {
            // Background task — surface only in logs. The existing fallback
            // heuristic title/summary stays in place when this fails.
            print("⚠️ Conversation metadata generation failed: \(error.localizedDescription)")
        }
    }

    /// Renders the chosen exchanges into a single text payload for the
    /// metadata model. Each side of an exchange is trimmed to 400 characters
    /// so the prompt stays well within Haiku's small-input sweet spot even
    /// when the user has rambled.
    private static func buildMetadataUserPrompt(from exchanges: [ConversationExchange]) -> String {
        let exchangeBlocks = exchanges.map { exchange -> String in
            let userText = exchange.userTranscript
                .trimmingCharacters(in: .whitespacesAndNewlines)
                .prefix(400)
            let assistantText = exchange.assistantResponse
                .trimmingCharacters(in: .whitespacesAndNewlines)
                .prefix(400)
            return "user: \(userText)\nsparkle: \(assistantText)"
        }
        return """
        here is the recent conversation between the user and sparkle. generate a title and summary that capture the topic of the whole exchange.

        \(exchangeBlocks.joined(separator: "\n\n"))
        """
    }

    /// Extracts the title and summary from a model response. Tolerates code
    /// fences and stray prose around a JSON object since small models will
    /// occasionally wrap their output despite explicit instructions.
    private static func parseMetadataJSON(from rawResponse: String) -> (title: String, summary: String)? {
        let jsonSubstring = extractFirstJSONObject(from: rawResponse) ?? rawResponse
        ...
        let title = (parsedObject["title"] as? String) ?? ""
        let summary = (parsedObject["summary"] as? String) ?? ""
        if title.isEmpty && summary.isEmpty { return nil }
        return (title: title, summary: summary)
    }

    /// Slices out the first balanced `{ ... }` block from a string so a JSON
    /// parser sees clean input even when the model adds prose or fences.
    private static func extractFirstJSONObject(from text: String) -> String? {
        guard let openBraceIndex = text.firstIndex(of: "{"),
              let closeBraceIndex = text[openBraceIndex...].lastIndex(of: "}")
        else { return nil }
        return String(text[openBraceIndex...closeBraceIndex])
    }
}
