...
struct Conversation: Codable, Identifiable {
    ...
    /// True once the user manually renames the title (or edits via the edit
    /// sheet). Auto-generated metadata regeneration must respect this so a
    /// background AI title call never clobbers a name the user chose. Optional
    /// for backwards compatibility with conversations saved before the flag
    /// existed — those decode as `nil`, which is treated as "free to update".
    ...
    init(title: String = "", summary: String = "", spaceId: UUID? = nil, toolType: ConversationToolType? = nil, lessonContext: String? = nil) {
        ...
        self.isTitleManuallyEdited = false
        self.exchanges = []
        self.createdAt = Date()
        self.updatedAt = Date()
    }
    ...
    var titleWasManuallyEdited: Bool { isTitleManuallyEdited ?? false }
...
final class ConversationStore: ObservableObject {
    ...
    func updateConversation(_ conversationId: UUID, title: String, summary: String, toolType: ConversationToolType) {
        ...
        // Any path through `updateConversation` is a user-initiated edit
        // (rename row, edit sheet, edit card). Mark the title as manually
        // owned so background AI metadata regeneration skips this row.
        conversations[idx].isTitleManuallyEdited = true
        save()
    }

    /// Updates a conversation's auto-generated title and summary if (and only
    /// if) the user has not manually renamed it. The user-facing edit paths
    /// flip `isTitleManuallyEdited` to true; this method respects that flag so
    /// AI-generated names never clobber user choices.
    ///
    /// Empty inputs are ignored — they likely mean the model returned a
    /// degenerate response and we'd rather keep the existing fallback than
    /// blank out a row.
    func applyAutoGeneratedMetadata(conversationId: UUID, title: String, summary: String) {
        guard let idx = conversations.firstIndex(where: { $0.id == conversationId }) else { return }
        if conversations[idx].titleWasManuallyEdited { return }

        let trimmedTitle = title.trimmingCharacters(in: .whitespacesAndNewlines)
        let trimmedSummary = summary.trimmingCharacters(in: .whitespacesAndNewlines)

        var didChange = false
        if !trimmedTitle.isEmpty, conversations[idx].title != trimmedTitle {
            conversations[idx].title = trimmedTitle
            didChange = true
        }
        if !trimmedSummary.isEmpty, conversations[idx].summary != trimmedSummary {
            conversations[idx].summary = trimmedSummary
            didChange = true
        }
